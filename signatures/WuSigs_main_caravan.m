...
out_filename = ['out_sigEvent_', caravan_data, '.csv'];
...
%___________________________________________________________________________________
% Read metadata
attrs_geo = readtable(fullfile(data_dir, caravan_dir, attributes_dir, caravan_data, ['attributes_other_', caravan_data, '.csv']));
...
for i = 1:numGauges
    try
        % Get the gauge id
        gauge_id = cell2mat(us_gauges(i, :).gauge_id);
        ...
        % Prepare TOSSH imput
        Q = num2cell(sync_data.streamflow,1); %mm/day
        t = num2cell(sync_data.date,1);
        P = num2cell(sync_data.total_precipitation_sum,1);
        P_hrfrac = num2cell(sync_data.max_hourly_frac, 1);  % Use hourly fraction
        % PET = num2cell(sync_data.potential_evaporation_sum_FAO_PENMAN_MONTEITH,1);
        % T = num2cell(sync_data.temperature_2m_mean,1);
