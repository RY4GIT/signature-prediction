function [IE_effect, SE_effect, IE_thresh_signif, IE_thresh, ...
    ...
    Storage_thresh, Storage_thresh_signif, min_Qf_perc, R_Pint_RC, R_Pvol_RC, SOF_effect, IOF_effect, Stor_effect ...
    error_flag, error_str, fig_handles] = sig_EventGraphThresholdsHourlyRain(Q,t,P,P_hrfrac,varargin)
...
min_Qf_perc = 100*(min(event_array(:,7)./event_array(:,1)));
...
%% ----Extras 
SOF_indices_qf = and(qf_vars(1:5),[1;0;0;0;0]);
SOF_var_numbers_qf = qf_vars(1:5).*cumsum(qf_vars(1:5));
SOF_coeff_numbers_qf = SOF_var_numbers_qf(SOF_indices_qf)+1;
SOF_effect = qf_coeffs(SOF_coeff_numbers_qf);
if numel(SOF_effect)==0;SOF_effect = NaN;end

IOF_indices_qf = and(qf_vars(1:5),[0;0;1;0;0]);
IOF_var_numbers_qf = qf_vars(1:5).*cumsum(qf_vars(1:5));
IOF_coeff_numbers_qf = IOF_var_numbers_qf(IOF_indices_qf)+1;
IOF_effect = qf_coeffs(IOF_coeff_numbers_qf);
if numel(IOF_effect)==0;IOF_effect = NaN;end

Stor_indices_qf = and(qf_vars(1:5),[0;0;0;0;1]);
Stor_var_numbers_qf = qf_vars(1:5).*cumsum(qf_vars(1:5));
Stor_coeff_numbers_qf = Stor_var_numbers_qf(Stor_indices_qf)+1;
Stor_effect = qf_coeffs(Stor_coeff_numbers_qf);
if numel(Stor_effect)==0;Stor_effect = NaN;end
