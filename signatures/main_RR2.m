...
% Choose which event separation method to use
method = 'tarasova';
...
cloud_dir = 'Y:\Gruppen\cathyd\mcmillan\signatures_processes\signature-prediction\Results'; % 'G:\Araki' on lab computer
data_dir = 'C:\Users\mcmillan\Documents\RR2';
caravan_dir = 'Caravan1.5';
attributes_dir = 'attributes';
flow_dir = 'Q';
rain_dir = 'P';
PET_T_dir = 'PET_T';
event_dir = 'event_char';
data_type = 'txt';
...
%___________________________________________________________________________________
%Read Gauge IDs
attrs_climate = readtable(fullfile(data_dir, 'all_gauges.xlsx'));
...
numGaugesTemp = numGauges;
for idx = 1:numGaugesTemp
%parfor idx = 1:numGauges
    
    try
        % Get the gauge id
        gauge_id = attrs_climate(idx, :).GaugeID;
        fprintf("Currently processing %s\n", num2str(gauge_id))
        
        %___________________________________________________________________________________
        % Data preparation
        % Load flow data and convert it to datetime table
        file_path = fullfile(data_dir, flow_dir, ...
            [num2str(gauge_id), '_qobs.', data_type]);
        data_flow = readtable(file_path);
        data_flow.date = datetime(data_flow.date, 'InputFormat', 'yyyy-MM-dd');
        data_flow_timetable = table2timetable(data_flow, 'RowTimes', 'date');
        %     disp(head(data_timetable));

        % Load rain data and convert it to datetime table
        file_path = fullfile(data_dir, rain_dir, ...
            [num2str(gauge_id), '_Pobs.', data_type]);
        data_rain = readtable(file_path);
        data_rain.date = datetime(data_rain.date, 'InputFormat', 'yyyy-MM-dd');
        data_rain_timetable = table2timetable(data_rain, 'RowTimes', 'date');


        % Load PET, T data and convert it to datetime table
        file_path = fullfile(data_dir, PET_T_dir, ...
            [num2str(gauge_id), '_.', data_type]);
        data_pet_t = readtable(file_path);
        data_pet_t.Date = datetime(data_pet_t.Date, 'InputFormat', 'dd/MM/yyyy');
        data_pet_t_timetable = table2timetable(data_pet_t, 'RowTimes', 'Date');

        % Load event file and convert to table
        file_path = fullfile(data_dir, event_dir, ...
            [num2str(gauge_id), '_single.', data_type]);
        event_table_single = readtable(file_path);

        % Load event file with double events and convert to table
        file_path = fullfile(data_dir, event_dir, ...
            [num2str(gauge_id), '_double.', data_type]);
        event_table_double = readtable(file_path);  

        %Combine events
        event_table = [event_table_single; event_table_double];
        event_table = sortrows(event_table, 'dd_mm_yyyy');

        % Prepare TOSSH imput
        Q = num2cell(data_flow.qobs,1); %mm/day
        t = num2cell(data_flow.date,1);
        P = num2cell(data_rain.Pobs,1);
        PET = num2cell(data_pet_t.PET,1);
        T = num2cell(data_pet_t.T,1);
        ...
        switch sig_cat
            ...
            case 'calc_All_custom'
                    signatures = calc_All_custom_eventmethod(Q, t, P, PET, T,...
                        ...
                        'filter_par', recession_param.filter_par, ...
                        'method', method, ...
...
%         doing vertcat
        ...
        % Create an empty output
            fieldNames = {'BaseflowRecessionK', 'BaseflowRecessionK_error_str', ...
                'BFI', 'BFI_error_str', ...
                'EventRR', 'EventRR_error_str', ...
                'RecessionParameters_a', 'RecessionParameters_b', 'RecessionParameters_T0', 'RecessionParameters_error_str', ...
                'Recession_a_Seasonality', 'Recession_a_Seasonality_error_str', ...
                'TotalRR', 'TotalRR_error_str', ...
                'VariabilityIndex', 'VariabilityIndex_error_str', ...
                'IE_effect', 'SE_effect', ...
                ...
                'AverageStorage', 'AverageStorage_error_str'};
...
results.gauge_id = attrs_climate.GaugeID(1:numGaugesTemp);
