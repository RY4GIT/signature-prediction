...
%path where hourly data is held
hr_path = 'C:\Users\hmcmillan\Google Drive New\InUse\TOSH_toolbox\CAMELS_MATLAB\HourlyRain';
...
load('C:\Users\hmcmillan\Google Drive New\InUse\TOSH_toolbox\CAMELS_MATLAB\Data\CAMELS_data.mat')
...
for i = 1:n_CAMELS 
    ...
    %Extract precip max hourly fraction 
    hr_full_path = fullfile(hr_path,strcat(num2str(CAMELS_data.gauge_id(i),'%08d'),"_hourly_nldas.mat"));
    load(hr_full_path)
    hr_indices = 1:height(tt_max_hourly_frac); 
    hr_start_ind = hr_indices(tt_max_hourly_frac.date==datetime(1989,10,1));
    if isempty(hr_start_ind); hr_start_ind = 1; end 
    hr_end_ind = hr_indices(tt_max_hourly_frac.date==datetime(2009,9,30));    
    P_hr_frac = tt_max_hourly_frac.max_hourly_frac(hr_start_ind:hr_end_ind);
    
    t_mat{i} = t;
    Q_mat{i} = Q;
    P_mat{i} = P;
    P_hrfrac_mat{i} = P_hr_frac;
    PET_mat{i} = PET;
    
end

save(fullfile(hr_path,'hourly_arrays.mat'),"t_mat","Q_mat","P_mat","P_hrfrac_mat","PET_mat")
