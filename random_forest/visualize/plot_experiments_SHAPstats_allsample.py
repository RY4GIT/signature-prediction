...
for process in df_group_all["Process"].unique():
    ...
    df_group_avg = (
        ...
        .reset_index()
    ).rename(columns={"mean_phi_abs_perc": f"{stat}_phi_abs_perc"})
    df_group_avg["Process"] = process

    # For each gauge_id, get the row with the maximum phi_abs_perc
    _df_group_avg_max = (
        df_group_avg.loc[
            df_group_avg.groupby("gauge_id")[f"{stat}_phi_abs_perc"].idxmax()
        ][
            [
                "gauge_id",
                f"{stat}_phi_abs_perc",
                "Group",
                "gauge_lon",
                "gauge_lat",
                "count",
        ...
        .rename(
            columns={
                f"{stat}_phi_abs_perc": f"max_{stat}_phi_abs_perc",
                "Group": "Group_max",
            }
        )
...
def plot_shap_in_map_max(
    ...
    max_opacity = df[varname].quantile(0.75)
