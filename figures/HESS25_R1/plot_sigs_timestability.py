...
_plot_config = pd.read_csv(
    "plot_sigs_config_timestability.csv",
)

# %% Make a shorter list of signatures to plot
...
# Filter to only desired signatures and order by the specified list
plot_config = _plot_config[_plot_config["column_name"].isin(desired_columns)].copy()
plot_config["__order"] = pd.Categorical(
    plot_config["column_name"], categories=desired_columns, ordered=True
)
plot_config = plot_config.sort_values("__order").drop(columns="__order")


# %%

attrs_geo = pd.read_csv(
    os.path.join(
        data_dir,
        caravan_dir,
        attributes_dir,
        caravan_data,
        f"attributes_other_{caravan_data}.{data_type}",
    )
)
...
gauge_ids = attrs_geo["gauge_id"][
    attrs_geo["gauge_id"].str.startswith(f"{caravan_data}_")
].tolist()

gauge_ids_rnd = random.sample(gauge_ids, 10)
...
for i, gauge_id in enumerate(
    gauge_ids
):  # Choose gauge_ids_rnd (smaller set) or gauge_ids (all gauges)
    # ______________________________________________________________________________________________
    # Get the data
    ...
    for ax, (index, row) in zip(axes, plot_config.iterrows()):
        try:
            ax.plot(
                sigs["duration"], sigs[row["column_name"]], alpha=0.3, color="tab:blue"
            )
...
fig.savefig(
    os.path.join(
        home_dir, sig_output_dir, camels_results_dir, "sig_vs_duration_selected.png"
    )
)
