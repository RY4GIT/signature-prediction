...
########################################################################################
#
# Plot the bivariate map (highlight for 1-1 class)
...
def plot_bivariate_map(df, sig1, sig2, fig_dir, plot_mode="polygon"):
    ...
    if plot_mode == "scatter":
        ...
        df_highlight = df[
            df["color"].isin(["#159DD0", "#15869E", "#60979F", "#5FB2D1"])
        ].copy()
        df_not_highlight = df[
            ~df["color"].isin(["#159DD0", "#15869E", "#60979F", "#5FB2D1"])
        ].copy()

        if len(df_not_highlight) > 0:
            # Plot the polygons
            df_not_highlight.plot(
                ax=ax,
                color=df_not_highlight["color"],
                linewidth=0.2,
                alpha=0.4,
                zorder=98,
            )
        ...
        if len(df_highlight) > 0:
            # If the color is one of the colors for the high quantile classes, highlight it
            # Plot the polygons with a black outline
            df_highlight.plot(
                ax=ax,
                color=df_highlight["color"],
                linewidth=2.0,
                alpha=0.4,
                zorder=99,
                edgecolor="black",
    ...
    plt.savefig(
        os.path.join(
            fig_bivar_dir,
            f"agu25_bivar_{sig1.column_name}_{sig2.column_name}_{plot_mode}.png",
        ),
        dpi=300,
    )
...
def create_bivariate_legend(colors, x_label, y_label, x_ticks, y_ticks, fig_dir):
    ...
    plt.savefig(
        os.path.join(
            fig_processes_dir,
            f"agu25_bivar_{sig1.column_name}_{sig2.column_name}_legend.png",
        )
    )
...
def plot_process_dominance_map(df_sigs, plot_sigs_config, fig_dir):
    ...
    plt.savefig(
        os.path.join(
            fig_dominant_process_dir, f"fig_dominant_process_agu25_slide{slide_n}.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )
