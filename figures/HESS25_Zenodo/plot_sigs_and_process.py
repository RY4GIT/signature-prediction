...
gg2_attrs_file = r"G:\Shared drives\Signatures -- large scale\baseflow\RAraki\data\derived_attrs\assembled_RA\attrs_gages2_based_20250517.csv"
...
gg2_attrs.rename(columns={"STAID": "gauge_num"}, inplace=True)
gg2_attrs = gg2_attrs.dropna(subset=["gauge_name"])
# %%
attrs = pd.concat(
    [
        attrs_caravan,
        gg2_attrs[["gauge_name", "country", "gauge_lat", "gauge_lon", "area"]],
    ],
    axis=0,
)
attrs
...
# %% ##################################
#
# the process dominance
#
##################################


# %%
def create_process_dominance_map(
    df_sigs, plot_sigs_config, fig_dir, climate_cluster_shp
):
    df_sigs_clean = df_sigs.copy()
...
create_process_dominance_map(df_sigs_clean, plot_sigs_config, zenodo_dir)
