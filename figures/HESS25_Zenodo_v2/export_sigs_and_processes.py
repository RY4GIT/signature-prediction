...
local_dir = r"E:\data"
cara_1p4_dir = (
    r"G:\Shared drives\Signatures -- large scale\baseflow\AHolt\data\Caravan_1.4"
)

# Output directory (For signatures results, change name to match the current run dates)
out_dir = os.path.join(gdrive_dir, "out", "signatures", "caravan_us_20260716")
out_dir_gages2 = os.path.join(gdrive_dir, "out", "signatures", "gages2_20260716")
rf_out_dir = os.path.join(gdrive_dir, "out", "rf", "output_raraki_20260729_cluster_all")

zenodo_dir = os.path.join(gdrive_dir, "out", "zenodo_v2", "data")
...
wspolygon_camels_file = os.path.join(
    cara_1p4_dir, "shapefiles", "camels", "camels_basin_shapes.shp"
)
wspolygon_camels = gpd.read_file(wspolygon_camels_file).to_crs(epsg=4326)
wspolygon_hysets_file = os.path.join(
    cara_1p4_dir, "shapefiles", "hysets", "hysets_basin_shapes.shp"
)
